<div class="{{ $col }}" id="{{ $name }}-hidden" {{ $hidden ? 'hidden=hidden' : '' }}>
    <div class="form-group">
        ...
        @endif
        <textarea id="{{ !empty($invalid) ? $invalid : $name }}" class="form-control {{ $ckeditor ? 'ckeditor' : '' }} @error(!empty($invalid) ? $invalid : $name) is-invalid @enderror" name="{{ $name }}" rows="{{ $rows }}">{!! old($name, $value) !!}</textarea>
