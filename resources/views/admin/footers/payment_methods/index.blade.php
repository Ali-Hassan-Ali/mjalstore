<x-admin.layout.app>
    ...
    <x-slot name="title">{{ trans('menu.payment_methods') }}</x-slot>

    <div><h2>{{ trans('menu.payment_methods') }}</h2></div>
    ...
    <ul class="breadcrumb mt-2">
        ...
        <li class="breadcrumb-item">{{ trans('menu.payment_methods') }}</li>
    </ul>
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-md-12">
                        ...
                        @if(permissionAdmin('create-payment_methods'))
                            <a href="{{ route('admin.footers.payment_methods.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> @lang('admin.global.create')</a>
                        @endif

                        @if(permissionAdmin('create-payment_methods'))
                            <form method="post" action="{{ route('admin.footers.payment_methods.bulk_delete') }}" style="display: inline-block;">
                                @csrf
                                @method('delete')
